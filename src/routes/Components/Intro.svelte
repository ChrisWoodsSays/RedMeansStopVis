<script lang="ts">
    ...
    $: margin = { 
		top: 50, 
		phylloRight: 50, textRight: 50, 
		bottom: 50, 
		phylloLeft: 50, textLeft: 50  }
	...
	$: phylloScale = d3
		...
		.range([0, Math.min(width/2 - margin.phylloLeft - margin.phylloRight, height * 2/3)]);
	$: xScale = d3
		.scaleLinear()
		.domain([0,12])
		.range([0, width/2 - margin.textLeft - margin.textRight]);
	$: yScale = d3
		.scaleLinear()
		.domain([0,12])
	...
	onMount(
		async () => {
			points = getStopSignPoints(1, 5, [], deaths2022, "", rmsRed );
		}
	)
	let innerWidth=0
...
>
	...
	<svg viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg">
		<g transform="translate({margin.phylloLeft}, {margin.top})">
			{#each points as p, i}
				<circle
					cx = {phylloScale(p.x)} cy = {phylloScale(p.y)} r={circleScale(p.radius)} fill = {p.colour}>
				</circle>
			{/each}
			<text class = "annotation" x = {(phylloScale.range()[1]- phylloScale.range()[1])/2} y = {phylloScale.range()[1]+50}>Each circle represents one of the {deaths2022.toLocaleString()} deaths on British roads in 2022.</text>
		</g>
		<g class = "title" transform="translate({margin.textLeft + width/4}, {margin.top+50})">
			<text x = {xScale(6)} y = {yScale(2)} class = "highlight">RED</text>
			<text x = {xScale(6)} y = {yScale(4.5)}>means</text>
			<text x = {xScale(6)} y = {yScale(7.5)} class = "highlight">STOP</text>
		</g>
	</svg>
</div>
